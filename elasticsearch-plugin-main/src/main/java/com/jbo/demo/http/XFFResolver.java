...
public class XFFResolver implements ConfigChangeListener {
    ...
    public void onChange(String event, Settings settings) {
        ...
        enabled = settings.getAsBoolean("demo.dynamic.http.xff.enabled", true);
        if(enabled) {
            detector = new RemoteIpDetector();
            detector.setInternalProxies(settings.get("demo.dynamic.http.xff.internalProxies", detector.getInternalProxies()));
            detector.setProxiesHeader(settings.get("demo.dynamic.http.xff.proxiesHeader", detector.getProxiesHeader()));
            detector.setRemoteIpHeader(settings.get("demo.dynamic.http.xff.remoteIpHeader", detector.getRemoteIpHeader()));
            detector.setTrustedProxies(settings.get("demo.dynamic.http.xff.trustedProxies", detector.getTrustedProxies()));
